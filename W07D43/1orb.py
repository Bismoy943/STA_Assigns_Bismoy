    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.current_time > self.secondcandle_stop.time() and self.current_time <= self.target_end.time():
                ...
                if (self.dataopen[0]-self.ORH>=self.params.exitbars*(self.ORH-self.ORL)) or (self.datahigh[0]-self.ORH>=self.params.exitbars*(self.ORH-self.ORL)) or (self.datalow[0]-self.ORH>=self.params.exitbars*(self.ORH-self.ORL)) or (self.dataclose[0]-self.ORH>=self.params.exitbars*(self.ORH-self.ORL)):
                    for open_order in [o for o in self.broker.orders if o.status < 4]:
                        self.cancel(open_order)
                    self.close()
