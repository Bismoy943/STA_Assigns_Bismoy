...
class FirstStrategy(bt.Strategy):
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            ...
            if order.isbuy():
                ...
                self.log(f"SELL STOP EXECUTED/SELL EXITED,Price: {order_details}")
    ...
    def __init__(self):
        ...
        self.target_end = datetime.now().replace(hour=14, minute=55, second=0, microsecond=0)
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.current_time>self.secondcandle_stop.time() and self.current_time<=self.evening_end.time():
                if self.sellorder is None and self.sold_today==False:


                    self.sellorder=self.sell(exectype=bt.Order.Stop,price=self.ORL-1,valid=0)
            ...
            if self.current_time > self.secondcandle_stop.time() and self.current_time <= self.target_end.time():

                if (self.ORL-self.dataopen[0]>=self.params.exitbars*(self.ORH-self.ORL)) or (self.ORL-self.datahigh[0]>=self.params.exitbars*(self.ORH-self.ORL)) or (self.ORL-self.datalow[0]>=self.params.exitbars*(self.ORH-self.ORL)) or (self.ORL-self.dataclose[0]>=self.params.exitbars*(self.ORH-self.ORL)):
                    for open_order in [o for o in self.broker.orders if o.status < 4]:
                        self.cancel(open_order)
                    self.close()
    ...
    def stop(self):
        self.log("RR set:"+str(self.params.exitbars))
        self.log("Final portfolio:"+str(self.broker.getvalue()))
...
if __name__=='__main__':
    ...
    cerebro.optstrategy(FirstStrategy,exitbars=range(1,21))
