...
def runcerebro(cerebro):
    runstrat=cerebro.run(maxcpus=1)
    strat=runstrat[0]
    ar=strat.analyzers.getbyname('myannualreturn')
    annualreturnanalyzer=open(r'C:/Users/Sonu/tradecoder/STA_Assigns_Bismoy/W07Remaining/output/annualreturnanalyzer.txt','w+')
    drawdownanalyzer = open(
        r'C:/Users/Sonu/tradecoder/STA_Assigns_Bismoy/W07Remaining/output/drawdownanalyzer.txt', 'w+')
    returnsanalyzer = open(
        r'C:/Users/Sonu/tradecoder/STA_Assigns_Bismoy/W07Remaining/output/returnsanalyzer.txt', 'w+')
    sqnanalyzer = open(
        r'C:/Users/Sonu/tradecoder/STA_Assigns_Bismoy/W07Remaining/output/sqnanalyzer.txt', 'w+')
    tradeanalyzer = open(
        r'C:/Users/Sonu/tradecoder/STA_Assigns_Bismoy/W07Remaining/output/tradeanalyzer.txt', 'w+')
    vwranalyzer = open(
        r'C:/Users/Sonu/tradecoder/STA_Assigns_Bismoy/W07Remaining/output/vwranalyzer.txt', 'w+')
    annualreturnanalyzer.write(json.dumps(ar.get_analysis()))
    drawdownanalyzer.write(json.dumps(strat.analyzers.getbyname('mydrawdown').get_analysis()))
    returnsanalyzer.write(json.dumps(strat.analyzers.getbyname('myreturns').get_analysis()))
    sqnanalyzer.write(json.dumps(strat.analyzers.getbyname('mySQN').get_analysis()))
    tradeanalyzer.write(json.dumps(strat.analyzers.getbyname('mytradeanalyzer').get_analysis()))
    vwranalyzer.write(str(strat.analyzers.getbyname('myvwr').get_analysis()))
    print("Max Drawdown:")
    pprint.pprint(strat.analyzers.getbyname("mydrawdown").get_analysis().max.drawdown)
    pprint.pprint(strat.analyzers.getbyname("mydrawdown").get_analysis())
    pprint.pprint(strat.analyzers.getbyname("myannualreturn").get_analysis())
    printTradeAnalysis(strat.analyzers.mytradeanalyzer.get_analysis())
    printSQN(strat.analyzers.mySQN.get_analysis())
...
def printTradeAnalysis(analyzer):
    ...
    print("Average win in rupees:",average_won)
    print("Total loss:",total_lost)
    print("Average loss in rupees:",average_lost)
    print("PNL Net:",pnl_net)
    print("Strike rate/Win rate:",strike_rate)
    print("--------------------------")
...
if __name__=='__main__':
    ...
    cerebro.addsizer(bt.sizers.FixedSize, stake=9)
    #cerebro.broker.setcommission(commission=0.001)
    cerebro.broker.set_cash(1000000.00)
    cerebro.addobserver(bt.observers.Trades)
    cerebro.addobserver(bt.observers.DrawDown)
    cerebro.addobserver(bt.observers.BuySell)
    cerebro.addwriter(bt.WriterFile,csv=True,out="C:/Users/Sonu/tradecoder/STA_Assigns_Bismoy/W07Remaining/output"+dt.datetime.now().strftime("%H")+".csv")
